...
namespace nirmana
{
    public partial class MainForm
    {
        ...
        private void BuildTimelinePanel()
        {
            ...
            Label lblClip = new Label { Text = "Clip:", AutoSize = true, Margin = new Padding(8, 10, 2, 0) };
            Theme.StyleLabel(lblClip, secondary: true);

            _clipCombo = new ComboBox { Width = 160, DropDownStyle = ComboBoxStyle.DropDownList, Margin = new Padding(2, 6, 6, 0) };
            Theme.StyleComboBox(_clipCombo);
            ...
            Theme.StyleSecondaryButton(_btnNewClip);
            ...
            Theme.StyleSecondaryButton(_btnDeleteClip);
            ...
            Panel row2 = new Panel { Dock = DockStyle.Fill, BackColor = Theme.Panel };

            _btnInsertKeyframe = new Button { Text = "Insert Keyframe (I)", AutoSize = true, Location = new Point(8, 6) };
            Theme.StyleButton(_btnInsertKeyframe);
            _btnInsertKeyframe.Click += (s, e) => { InsertKeyframe(); _glControl.Focus(); };

            _btnPlayStop = new Button { Text = "Play", Width = 64, Location = new Point(156, 6) };
            Theme.StyleButton(_btnPlayStop);
            _btnPlayStop.Click += (s, e) => { TogglePlayback(); _glControl.Focus(); };

            _lblTime = new Label { Text = "0.0s / 0.0s", AutoSize = true, Location = new Point(230, 13) };
            Theme.StyleLabel(_lblTime, secondary: true);

            _timeline = new ModernSlider
            {
                Minimum = 0,
                Maximum = 50,
            ...
            _timeline.ValueChanged += (s, e) =>
            {
                _playbackTime = _timeline.Value / 10f;
                ApplyPoseAtCurrentTime();
                UpdateTimeLabel();
            };
            _timeline.DragEnded += (s, e) => _glControl.Focus();
        ...
        {
            // Warna background viewport 3D & grid SENGAJA tidak diubah (dipakai
            // apa adanya) — beda dengan UI di sekitarnya yang sekarang pakai
            // tema ungu gelap lewat Theme.cs.
